...
export function SingleSelectAutocomplete(
    {title, description, placeholder, items, setSelection} 
    : {title: string, description: string, placeholder: string, items: string[], setSelection: (item: string) => void}
) {
    ...
    return (
        <div className="space-y-2">
            ...
            <Autocomplete
                ...
                onSelectionChange={(k) => (
                    k !== null && values.has(k.toString()) && setSelection(values.get(k.toString())!)
                )}
            >
                {keys.map(key => (
                    <AutocompleteItem key={key.toString()}>{values.get(key.toString())}</AutocompleteItem>
                ))}
            </Autocomplete>
        </div>
    );
