 ...
 * Fetching all members from the database sounds like a very bad idea, but we would need some kind of
 * lazy loading for MultiSelectAutocomplete otherwise that is currently in todo
...
export function useFetchMembers(): SWRResponse<MemberT[], Error> {
  return useSWR(baseUrl + "/members", async (...args) => {
    const response = await fetch(...args, { next: { revalidate: 5 } });
